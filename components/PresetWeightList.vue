...
const allItems = props.weights ? Object.entries(props.weights as StringObject).map(([k, v]) => ({key: k, label: settings[k]?.name ?? k, content: parseValues(v as Record<string, number>) })) : [];
const searchValue = ref('')
const items = computed(() => {
  return searchValue.value && searchValue.value.length > 0 ? allItems.filter(item => (settings[item.label]?.name ?? item.label).toLocaleLowerCase().includes(searchValue.value.toLocaleLowerCase())) : allItems;
})
</script>
