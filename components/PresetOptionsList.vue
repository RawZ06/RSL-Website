...
const items = props.options ? Object.entries(props.options as StringObject).map(([k, v]) => ({key: k, label: settings[k]?.name ?? k, content: v})) : [];
</script>
    ...
    >
      <template #item="{ item }">
        ...
        <span v-else-if="item.key === 'conditionals'">
          <UTable :rows="Object.entries(item.content).map(([k, v]) => ({conditional: k, status: (v as Array<string>)[0], other: (v as Array<string>).slice(1).join(',')}))">
